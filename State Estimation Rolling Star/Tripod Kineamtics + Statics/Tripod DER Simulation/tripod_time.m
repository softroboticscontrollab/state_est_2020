function [vertex_xy,foot_data] = tripod_time(simdata,time)
...
num_v_per_foot = 5; % number of vertices per "flat part" of a limb.
...
limb_x = vertex_x(1:(num_limbs * num_v_per_limb)); %x-coordinates for the curved sections
limb_y = vertex_y(1:(num_limbs * num_v_per_limb)); %y-coordinates for the curved sections

%create data structure for curved sections
for i = 1:num_limbs
    if i == 1
        circ_x_single_limb = limb_x((i-1)*num_v_per_limb+1:(i*num_v_per_limb));
        circ_y_single_limb = limb_y((i-1)*num_v_per_limb+1:(i*num_v_per_limb));
        circ_x_single_limb = [limb_x(num_limbs * num_v_per_limb),circ_x_single_limb];
        circ_y_single_limb = [limb_y(num_limbs * num_v_per_limb),circ_y_single_limb]; 
    else
        %x and y coordinates for points on the for the curved section of the ith limb
        circ_x_single_limb = limb_x((i-1)*num_v_per_limb+1:(i*num_v_per_limb));
        circ_y_single_limb = limb_y((i-1)*num_v_per_limb+1:(i*num_v_per_limb));
        circ_x_single_limb = [limb_x((i-1)*num_v_per_limb),circ_x_single_limb];
        circ_y_single_limb = [limb_y((i-1)*num_v_per_limb),circ_y_single_limb]; 

    end
    %create data structure of x and y coordinates for each limb
    limb_data{i} = [circ_x_single_limb; circ_y_single_limb];
end

stright_x = vertex_x((num_limbs * num_v_per_limb)+1:end); %x-coordinates for the curved sections
stright_y = vertex_y((num_limbs * num_v_per_limb)+1:end); %y-coordinates for the curved sections
...
for i = 1:num_limbs
    ...
    stright_x_single_limb = [limb_data{i}(1,1), stright_x_single_limb];
    stright_y_single_limb = [limb_data{i}(2,1), stright_y_single_limb]; 
